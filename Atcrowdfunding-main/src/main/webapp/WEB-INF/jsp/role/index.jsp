...
<body>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<button type="button" class="btn btn-primary"
							style="float: right;"
							onclick="window.location.href='${APP_PATH }/role/toAdd.htm'">
	...
	<script type="text/javascript">
						...
						success : function(result) {
							...
							if (result.success) {
										...
										.each(
												...
												function(i, n) {
													...
													content += '  <td>'
															+ n.name + '</td>';
													content += '  <td>';
													content += '      <button type="button" onclick="window.location.href=\'${APP_PATH}/role/assignPermission.htm?roleid='
															+ (n.id)
															+ '\'" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
													content += '      <button type="button" onclick="window.location.href=\'${APP_PATH}/role/toUpdate.htm?id='
															+ (n.id)
															+ '\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
													content += '	    <button type="button" onclick="deleteRole('
															+ n.id
															+ ',\''
															+ n.name
								...
								$("#Pagination").pagination(page.totalsize, {
									...
									callback : queryPageRole,//查询当前页的数据 
		...
		function deleteRole(id, name) {
			...
			layer.confirm("确认要删除这个[" + name + "]用户吗?", {
				icon : 3,
				title : '提示'
			}, function(cindex) {
				$.ajax({
		...
		$("#deleteBatchBtn").click(function() {
			...
			$.each(selectCheckbox, function(i, n) {
				jsonObject["datasRole[" + i + "].id"] = n.id;
