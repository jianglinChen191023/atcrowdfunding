...
<body>
	...
	<div class="container theme-showcase" role="main">
		...
		<form id="uploadCertForm" method="post" enctype="multipart/form-data">
			<c:forEach items="${queryCertByAccttype }" var="cert"
				varStatus="status">
				<div class="form-group">
					<label for="exampleInputEmail1">${cert.name }</label> <input
						name="memberCertImgs[${status.index }].certid" value="${cert.id }"
						type="hidden"> <input
						name="memberCertImgs[${status.index }].imgfile" type="file"
						class="form-control"> <br> <img style="display: none"
						src="">
				</div>
			</c:forEach>

			<button type="button"
				onclick="window.location.href='${APP_PATH }/member/basicinfo.htm'"
